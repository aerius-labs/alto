...
pub fn parse_notarized(identity: Vec<u8>, bytes: Vec<u8>) -> JsValue {
    ...
    let notarized_js = NotarizedJs {
        proof: ProofJs {
            view: notarized.proof.proposal.round.view().get(),
...
pub fn parse_finalized(identity: Vec<u8>, bytes: Vec<u8>) -> JsValue {
    ...
    let finalized_js = FinalizedJs {
        proof: ProofJs {
            view: finalized.proof.proposal.round.view().get(),
...
pub fn leader_index(seed: JsValue, participants: usize) -> usize {
    ...
    let round = Round::new(Epoch::new(EPOCH), View::new(seed.view));
    
    // Implement Random::select_leader logic directly since elector module may not be available in WASM
    let n = participants;
    if round.view().get() == 1 {
        // Standard round-robin for view 1
        (round.epoch().get().wrapping_add(round.view().get()) as usize % n) as usize
    } else {
        // Use the seed signature as a source of randomness
        modulo(signature.encode().as_ref(), n as u64) as usize
    }
}
